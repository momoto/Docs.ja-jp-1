...
ASP.NET Core 2.1 の新機能の概要について [ASP.NET Core 2.1 の新機能](xref:aspnetcore-2.1) を参照してください。
...
* パッケージ参照を置き換える`Microsoft.AspNetCore.All`パッケージの参照で`Microsoft.AspNetCore.App`します。 削除された依存関係を追加する必要があります`Microsoft.AspNetCore.All`します。 詳細については、<xref:fundamentals/metapackage#migrate> および <xref:fundamentals/metapackage-app> を参照してください。 .NET Framework を対象とするアプリの場合。
  * メタパッケージ参照の代わりに個々 のパッケージ参照を追加します。
  * 2.1 には、各パッケージ参照を更新します。
...
## <a name="rules-for-projects-targeting-the-shared-runtime"></a>共有ランタイムを対象とするプロジェクトの規則

ASP.NET Core には、次の共有ランタイムが含まれています。
...
共有ランタイムを対象とするプロジェクトの既知の問題:

* 使用する`Microsoft.AspNetCore.App`と Entity Framework Core 2.1.1 2.1.0 とパッケージの競合によって NuGet 復元が失敗します。 推奨されるソリューションは、アップグレードする`Microsoft.AspNetCore.App`2.1.1 にします。 詳細については、次を参照してください。 [Microsoft.AspNetCore.App と依存関係を共有するパッケージがパッチ バージョンを参照できません](https://github.com/aspnet/Universe/issues/1180)します。
...
* 上記の利点はいるでしょう**いない**への変換を揃え、RCL バージョン。
...
* Identity UI 2.0 コードを変更せずに使用します。 Identity UI 2.0 コードを使用することがサポートされます。 これは、生成された Id コードを大幅に変更を行ったときに適切なアプローチです。
...
* 生成された Identity scaffolder を削除`IdentityDbContext`derrived クラスで、 *領域/ユーザー/データ/* フォルダー。
